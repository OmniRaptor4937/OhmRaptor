...
class GoslingAgent(BaseAgent):
    ...
    def get_closest_large_boost(self):
        ...
        for boost in available_large_boosts:
            ...
            if closest_large_boost is None or distance < closest_large_distance:
                closest_large_boost = boost
                closest_large_distance = distance
        return closest_large_boost

    #closest small boost pad
    def get_closest_small_boost(self):
        available_small_boosts = [boost for boost in self.boosts if boost.small and boost.active]
        closest_small_boost = None
        closest_small_distance = 10000
        for boost in available_small_boosts:
            distance = (self.me.location - boost.location).magnitude()
            if closest_small_boost is None or distance < closest_small_distance:
                closest_small_boost = boost
                closest_small_distance = distance
        return closest_small_boost

    # intended to be used in smaller areas as to not go into a constant boost loop. 

    def get_boost_or_ball(self):
        available_small_boosts = [boost for boost in self.boosts if boost.small and boost.active]
        closest_small_boost = None
        closest_small_distance = 10000
        for boost in available_small_boosts:
            ...
            if closest_small_boost is None or distance < closest_small_distance:
                closest_small_boost = boost
                closest_small_distance = distance
        if (self.me.location - closest_small_boost.location).magnitude() < (self.me.location - self.ball.location).magnitude():
            return closest_small_boost
        if (self.me.location - closest_small_boost.location).magnitude() > (self.me.location - self.ball.location).magnitude():
